...
Событие сработает, если значение поля `Город` в анкете подписчика изменится на `Москва`.
