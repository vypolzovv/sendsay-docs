...
**Изменение данных** ([описание в API](https://sendsay.ru/api/api.html#Изменение-данных))&nbsp;&mdash; Событие сработает, когда изменятся данные подписчика. Здесь есть два варианта:
...
**Совпадение даты и времени** ([описание в API](https://sendsay.ru/api/api.html#Точное-время)) — участник сценария перейдёт на следующий шаг текущей ветки сценария, если попадёт на этот блок в указанную дату и время. Если дата и время попадания участника на шаг не совпадут с указанными, участник пойдёт по ветке «Нет».

<p align="center">
  <img width="50%" src={DateTimeStamp} alt="Date and time stamp" />
</p>

Здесь можно указать время, число месяца, конкретную дату или день недели. Например, если указана дата и время «До 20.02.2024 15:00», а участник сценария попадает на блок в 14:00 20 февраля 2024 года, он пойдёт дальше по текущей ветке сценария. Если же он попадёт на этот блок позже 15:00, он пойдёт по ветке «Нет».
