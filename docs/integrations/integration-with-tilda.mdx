...
Если вы хотите добавлять подписчиков неподтверждёнными, то ссылку для вёбхука нужно получить через [API-консоль](https://app.sendsay.ru/api/console). Для получения ссылки используйте запрос:
```
{
