...
recent_article:
  new: false
  theses:
    - Как добавлять подписчиков неподтверждёнными
    - Как предотвратить ошибку передачи данных
    - Как назвать поле с адресом на форме
---
...
Если вы хотите добавлять подписчиков неподтверждёнными, то ссылку для интеграции нужно получить через [API-консоль](https://app.sendsay.ru/api/console). Для получения ссылки используйте запрос:
```
{
